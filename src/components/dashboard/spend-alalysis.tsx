...
import { AlertTriangle, BadgeCheck, Bot, TrendingUp, TrendingDown, Loader2, X } from 'lucide-react';
...
interface SpendingAnalysisDialogProps {
  mealLogs: MealLog[];
}

type AnalysisState = 'idle' | 'loading' | 'success' | 'error';

export default function SpendingAnalysisDialog({ mealLogs }: SpendingAnalysisDialogProps) {
  ...
  return (
    <>
      <Button variant="outline" onClick={() => handleOpenChange(true)}>
        <Bot className="mr-2 h-4 w-4" /> Analyze Spending
      </Button>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex justify-center items-center p-3">
          <div className="relative grid w-full max-w-lg gap-4 border bg-background p-6 shadow-lg sm:rounded-lg rounded-xl">
            ...
            <button
              ...
              className="absolute right-4 top-4 z-10 p-[2px] opacity-70 border rounded-full ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
            >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className={`font-medium ${
                          result.isSuspicious ? 'text-destructive' : 'text-green-700 dark:text-green-500'
                        }`}>
